 // Prueba de reacción - JavaScript
    ...
    function saveRounds(){ localStorage.setItem('reaction_rounds', JSON.stringify(rounds)); }

    function updateStats(){
      if(rounds.length===0){ lastTimeEl.textContent='— ms'; bestTimeEl.textContent='— ms'; avgTimeEl.textContent='— ms'; historyList.innerHTML=''; return }
    ...
    function toIdle(){ state='idle'; screen.className='screen ready'; mainText.textContent='Toca para empezar'; subText.textContent='Presiona INICIAR o toca el área'; }
    ...
    function registerTap(){
      ...
      if(state==='go'){
        ...
        vibrate(40);
        state='idle';
        // pequeña animación: volver a ready
    ...
    // Eventos
    screen.addEventListener('click', (e)=>{ registerTap(); });
    screen.addEventListener('touchstart', (e)=>{ registerTap(); });
    startBtn.addEventListener('click', ()=> startRound());
    resetBtn.addEventListener('click', ()=>{
      if(!confirm('Borrar historial?')) return; rounds = []; saveRounds(); updateStats();
    });

    bestBtn.addEventListener('click', ()=>{
      const times = rounds.map(r=>r.time).filter(t=>typeof t==='number');
      if(times.length===0){ alert('No hay tiempos válidos todavía.'); return }
      const best = Math.min(...times);
      // compartir Web Share API si está disponible
      const text = `Mi mejor tiempo de reacción: ${best} ms. ¿Puedes superarlo?`;
      if(navigator.share){ navigator.share({title:'Prueba de Reacción', text}).catch(()=>{}); }
      else{ prompt('Copia y comparte tu mejor tiempo:', text); }
    });
